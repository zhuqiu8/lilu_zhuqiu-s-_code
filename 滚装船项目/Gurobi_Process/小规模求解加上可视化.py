...
# 参数
B = 3  # 品牌数量
J = [20 , 20, 6]  # 每个品牌的车辆数量 HMC、
K = 4  # 堆场数量
l = [[4.4, 4.4, 4.4, 4.4, 4.4, 4.4, 4.4, 4.4, 4.4, 4.4, 4.4, 4.4, 4.4, 4.4, 4.4,4.4, 4.4, 4.4, 4.4, 4.4], #HMC-20
     [4.5, 4.5, 4.5, 4.5, 4.5, 4.7, 4.7, 4.7, 4.7, 4.7,4.7, 4.7, 4.7, 4.7, 4.7, 4.0, 4.0, 4.0, 4.0, 4.0],
     [2, 4, 6, 5, 3, 4]]  # 每辆车的长度（每个[]表示一个品牌，里面的数字表示这个品牌车的长度）



w = [[2.2, 2.2, 2.2, 2.2, 2.2, 2.2, 2.2, 2.2, 2.2, 2.2, 2.2, 2.2, 2.2, 2.2, 2.2, 2.2, 2.2, 2.2, 2.2, 2.2], #HMC
...
A = [390*33, 60*14, 27*16, 40*14]  # 堆场总面积
L = [390, 60, 27, 40]  # 堆场长度（对应五个堆场的长度）
W = [33, 14, 16, 14]  # 堆场宽度（对应五个堆场的宽度）
alpha, beta = 1, 0  # 权重参数 alpha表示车辆集中度，beta表示面积未使用最低
M_large = 1000  # 大常数
...
if model.status == GRB.OPTIMAL:
    ...
    for k in range(K):
        ...
        ax.set_title(f"Yard {k}")
        ax.set_xlim(0, L[k])
        ax.set_ylim(0, W[k])
        ax.set_aspect('equal')
        ax.add_patch(patches.Rectangle((0, 0), L[k], W[k], edgecolor='black', fill=False))
        ...
        for i in range(B):
            for j in range(J[i]):
                if x[i, j, k].X > 0.5:
                    pos_x = sigma[i, j, k].X
                    pos_y = tau[i, j, k].X
                    width = w[i][j] if r[i, j].X < 0.5 else l[i][j]
                    height = l[i][j] if r[i, j].X < 0.5 else w[i][j]
                    ax.add_patch(patches.Rectangle((pos_x, pos_y), width, height, edgecolor='blue', facecolor='skyblue', alpha=0.7))
                    ax.text(pos_x + width / 2, pos_y + height / 2, f"B{i}C{j}", ha='center', va='center', fontsize=8)
