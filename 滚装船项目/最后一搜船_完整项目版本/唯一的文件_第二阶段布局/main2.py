...
class Bin:
    ...
    def place_rectangle(self, rect: Rectangle) -> bool:
        ...
        step = 0.3  # Define step size for placement to handle floating-point coordinates
...
if __name__ == "__main__":
    initial_file_path = r"D:\Github\RORO_Project\OROR-PROJECT\25_1_6测试下一艘船_原堆场\第二次模型的输入数据.xlsx"  # 初始 Excel 文件，包含堆场名称
    output_file_path = r"D:\Github\RORO_Project\OROR-PROJECT\25_1_6测试下一艘船_原堆场\唯一的文件_第一阶段预分配\第一阶段预分配\Result\result1.xlsx"   # 输出的 Excel 文件，包含堆场数据
    # 定义堆场尺寸和优先品牌
    # bin_sizes = [
    #     (9.2 ,200), #"Yard C1.1",
    #     (34 , 150), #"Yard C1.2"
    ...
    bin_sizes = get_bin_sizes(path=r"D:\Github\RORO_Project\OROR-PROJECT\25_1_6测试下一艘船_原堆场\第二次模型的输入数据.xlsx", sheet_name="Yard_Areas",
    ...
    rectangles = (
            ...
            + [Rectangle(2.2, 4.5, id=i, brand="KONA_HEV") for i in range(543)]
            + [Rectangle(2.3, 5.1, id=i, brand="SONATA_HEV") for i in range(237)]
            + [Rectangle(2.2, 4.4, id=i, brand="VENUE") for i in range(724)]
            + [Rectangle(2.0, 4.6, id=i, brand="NIRO") for i in range(339)]
            + [Rectangle(2.2, 4.7, id=i, brand="SELTOS") for i in range(332)]
            ...
            + [Rectangle(2.2, 4.9, id=i, brand="ELANTRA_HEV") for i in range(959)]
    )
